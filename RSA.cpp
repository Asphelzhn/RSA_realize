...
bignum* bignum_init() {
	...
	b->data = (unsigned int *)calloc(BIGNUM_CAPACITY, sizeof(word));
	return b;
}
...
void bignum_copy(bignum* source, bignum* dest) {
	...
	if (source->capacity > dest->capacity) {
		...
		dest->data = (unsigned int *)realloc(dest->data, dest->capacity * sizeof(word));
	}
...
void bignum_fromint(bignum* b, unsigned int num) {
	...
	if (b->capacity < b->length) {
		b->capacity = b->length;
		b->data = (unsigned int*)realloc(b->data, b->capacity * sizeof(word));
	}
	b->data[0] = num;
}
...
void bignum_add(bignum* result, bignum* b1, bignum* b2) {
	...
	if (n + 1 > result->capacity) {
		result->capacity = n + 1;
		result->data = (unsigned int*)realloc(result->data, result->capacity * sizeof(word));
	}
	for (i = 0; i < n; i++) {
		sum = carry;
...
void bignum_subtract(bignum* result, bignum* b1, bignum* b2) {
	...
	if (b1->length > result->capacity) {
		...
		result->data = (word*)realloc(result->data, result->capacity * sizeof(word));
	}
...
void bignum_divide(bignum* quotient, bignum* remainder, bignum* b1, bignum* b2) {
	...
	if (bignum_less(b1, b2)) { /* Trivial case, b1/b2 = 0 iff b1 < b2. */
		...
		if (quotient->capacity < b1->length) {
			...
			quotient->data = (unsigned int*)realloc(quotient->data, quotient->capacity * sizeof(word));
		}
		for (i = b1->length - 1; i >= 0; i--) {
		...
		if (quotient->capacity < n - m) {
			...
			quotient->data = (unsigned int*)realloc(quotient->data, (n - m) * sizeof(word));
		}
		...
		* we introduce a dummy zero word to artificially inflate it. */
		if (b1copy->length != n) {
			...
			if (b1copy->length > b1copy->capacity) {
				...
				b1copy->data = (unsigned int*)realloc(b1copy->data, b1copy->capacity * sizeof(word));
			}
			b1copy->data[n - 1] = 0;
		}
		...
		for (i = n - m - 1; i >= 0; i--) {
			...
			if (m + 1 > temp3->capacity) {
				temp3->capacity = m + 1;
				temp3->data = (unsigned int*)realloc(temp3->data, temp3->capacity * sizeof(word));
			}
			temp3->length = 0;
			for (j = 0; j <= m; j++) {
...
bignum *encodeMessage(int len, int bytes, char *message, bignum *exponent, bignum *modulus) {
	...
	* bignum_init() all of these */
	int i, j;
	bignum *encoded = (bignum *)calloc(len / bytes, sizeof(bignum));
